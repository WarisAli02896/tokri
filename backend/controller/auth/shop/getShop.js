...
exports.get_Shop_By_User_Id = async (req, res) => {
    ...
    if (reqData.body.User.user_id == reqData.id) {
        await Users.findAll({
            where: {
    ...
    } else if (req.body.User.user_id != req.id) {
        return res.status(StatusCodes.UNAUTHORIZED).json({
            data: {
                errorMessage: "User not authorized for requested data"
            }
        });
    }
}
...
exports.get_shop_by_id = async (req, res) => {
    ...
    if (reqData.body.User.user_id == reqData.user_id) {
        await Users.findOne({
            where: {
                user_id: reqData.body.User.user_id
            },
    ...
    } else if (req.body.User.user_id != req.user_id) {
        return res.status(StatusCodes.UNAUTHORIZED).json({
            data: {
                errorMessage: "User not authorized for requested data"
            }
